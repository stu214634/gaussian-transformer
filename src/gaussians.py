#https://github.com/VAST-AI-Research/TriplaneGaussian/blob/main/tgs/models/renderer.py#L214
...
class GaussianModel():
    ...
    def __init__(self, xyz, opacity, rotation, scaling, shs) -> None:
        self.xyz = xyz
    ...
    def save_ply(self, path):
        ...
        f_dc = self.shs[:, :1].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    ...
    def load_ply(self, path):
        ...
        assert len(extra_f_names)==3*(2) ** 2 - 3
        features_extra = np.zeros((xyz.shape[0], len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_extra = features_extra.reshape((features_extra.shape[0], 3, (2) ** 2 - 1))
        ...
        self.xyz = torch.tensor(xyz, dtype=torch.float, device="cuda")
        ...
        self.shs = torch.cat([features_dc, features_rest], 1)
        self.opacity = torch.sigmoid(torch.tensor(opacities, dtype=torch.float, device="cuda"))
        self.scaling = torch.exp(torch.tensor(scales, dtype=torch.float, device="cuda"))
        self.rotation = torch.tensor(rots, dtype=torch.float, device="cuda")
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            shs_view = pc.shs.transpose(1, 2).view(-1, 3, 4)
            dir_pp = (pc.xyz - viewpoint_camera.camera_center.repeat(pc.shs.shape[0], 1))
            dir_pp_normalized = dir_pp/dir_pp.norm(dim=1, keepdim=True)
            sh2rgb = eval_sh(1, shs_view, dir_pp_normalized)
